...
if (isset($_GET['do'])) {
  ...
  if ($do == "edit") {
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
      ...
      if ($gender == "") {
        $check = 0;
        $genderError = "hãy nhập giới tính";
      }
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
      ...
      // name validation 
      if (!preg_match("/^[a-zA-Z-' ]*$/", $name)) {
        $nameError = "Chỉ được phép sử dụng chữ cái và khoảng trắng";
        $check = 0;
      }
      // email validation 
      if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        $emailError = ("email không phải là địa chỉ email hợp lệ");
        $check = 0;
      }
          ...
          <div class="users-table table-wrapper">
            <table class="table table-striped" id="table1">
              ...
              <button class="btn btn-primary" style="float: right;margin:10px 50px 0px 10px;">
                <a href="manage_users.php?do=add">  Thêm người dùng </a>
              </button>
              <thead>
                <tr class="users-table-info">
                  
                  <th>Tên người dùng</th>
                  <th>Emal nguời dùng</th>
                  <th>Mật khẩu người dùng</th>
                  <th>Giới tính người dùng</th>
                  <th>Ngày tạo người dùng</th>
                  <th>Hoạt động</th>
                </tr>
              </thead>
              <tbody>
                    ...
                    <td>
                      <div class="table-data-feature">
                        <button class="btn btn-success" title="edit">
                          <a href="manage_users.php?do=edit&id=<?php echo $user['user_id'] ?>"> sửa lại </a>
                        </button>
                        <button class="btn btn-danger" title="delete">
                          <a href="manage_users.php?delete=<?php echo $user['user_id'] ?>"> xóa </a>
                        </button>
                      </div>
                    </td>
                  </tr>
